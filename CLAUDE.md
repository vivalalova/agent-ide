...
# 執行單一 fork 測試（解決記憶體問題）
pnpm test:single
```

### CLI 工具使用
```bash
# 透過 npm link 安裝本地開發版本
npm link
...
- **Node.js**: >= 20.0.0
- **包管理器**: 使用 pnpm（而非 npm）
- **TypeScript**: 使用 ES Module (`"type": "module"`)

### 測試配置重要事項
- **測試 timeout**: 所有測試預設 30 秒超時（配置在 `vitest.config.ts`）
- **記憶體管理**: 使用 `--expose-gc` 和 `--max-old-space-size=4096` 優化記憶體使用
- **並發控制**: `maxConcurrency: 3` 和 `singleFork: true` 減少記憶體負載
- **單一測試**: 執行單一測試時使用 `pnpm test:single` 避免記憶體問題
- **Worker 限制**: 最大 2 個 Worker 進程，使用 fork pool
...
### 開發前必讀
- **開發前必須閱讀**：
  - `CLAUDE.md` - 專案整體規範和架構設計
  - 對應模組的 `plan.md` - 詳細的實作計畫和技術細節
- **任務追蹤**：
  - 完成的功能和任務必須在對應的 `plan.md` 中打勾標記
  - 使用 `- [x]` 標記已完成項目
  - 使用 `- [ ]` 標記待完成項目
