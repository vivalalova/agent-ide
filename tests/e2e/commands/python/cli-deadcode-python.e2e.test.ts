/**
 * deadcode 命令 E2E 測試 - Python 專案
 * 基於 python-deadcode-test fixture
 */

import { describe, it, expect, beforeEach, afterEach } from 'vitest';
import { loadFixture, executeCLI, type FixtureContext } from '../../../helpers/index.js';

describe('CLI deadcode Python - 基於 python-deadcode-test fixture', () => {
  let fixture: FixtureContext;

  beforeEach(async () => {
    fixture = await loadFixture('python-deadcode-test');
  });
  ...
  describe('基本功能', () => {
    ...
    it('應該檢測到 Python dead code', async () => {
      const result = await executeCLI(
        ['deadcode', '--path', fixture.rootPath, '--format', 'json'],
        { memfs: fixture.memfs }
      );
  ...
  describe('Python 特定符號類型', () => {
    it('應該檢測未使用的 Python 函式', async () => {
      const result = await executeCLI(
        ['deadcode', '--path', fixture.rootPath, '--format', 'json'],
        { memfs: fixture.memfs }
      );
    ...
    it('應該檢測未使用的 Python 變數', async () => {
      ...
      const output = JSON.parse(result.stdout);
      const variables = output.items.filter((item: { type: string }) => item.type === 'variable');
