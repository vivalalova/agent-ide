 ...
 *
 * 缺陷：ES2022 私有欄位（`#secret`）完全不被 symbol extractor 收錄
 * （src/plugins/typescript/types.ts 的 getNodeName 與
 * src/plugins/typescript/symbol-extractor.ts 的 getSymbolIdentifierRange
 * 只處理 `ts.isIdentifier` / `ts.isStringLiteral`，未處理 `ts.isPrivateIdentifier`）。
 *
 * 影響：
 ...
 *
 * 對照組：一般用 `private` 修飾字的欄位（非 `#` 語法）現行可正常索引，
 * 用來確認本測試只鎖定 `#` 私有欄位語法，不影響既有 private 修飾字支援。
 */
...
describe('CLI 私有欄位（#secret）symbol extractor 缺陷 regression', () => {
  ...
  it('search 應找到私有欄位 #secret 的宣告', async () => {
    ...
    const boxResults = output.results.filter((r: any) => r.filePath.endsWith('box.ts'));
  ...
  it('find-references 應回傳 #secret 的宣告與 this.#secret 使用處', async () => {
    ...
    expect(output.definition.file).toContain('box.ts');
    expect(output.definition.line).toBe(2);

    const boxReferences = output.references.filter((r: any) => r.file.endsWith('box.ts'));
    expect(boxReferences.some((r: any) => r.line === 4)).toBe(true);
  });

  it('rename #secret -> #hidden 應同步改宣告與使用處並產出合法 TS', async () => {
    const result = await executeCLI(
      [
        'rename',
    ...
    const afterContent = await fixture.memfs.readFile(
      `${fixture.rootPath}/src/box.ts`,
      'utf-8'
    );

    // 宣告與使用處都應同步改名，且仍保留 `#` 私有欄位語法（合法 TS）
  ...
  it('對照組：一般 private 修飾字欄位（非 # 語法）現行可正常索引（防退化）', async () => {
    await fixture.writeFile('src/normal-private.ts', [
      'export default class NormalBox {',
      '  private normalSecret = 1;',
      '  reveal() {',
    ...
    const results = output.results.filter((r: any) => r.filePath.endsWith('normal-private.ts'));
  ...
  it('rename #secret -> #hidden 不應誤改其他檔案同名但無關的 .secret 屬性存取／欄位', async () => {
    const OTHER_SOURCE = [
      'export function readConfig(cfg: { secret: number }) {',
      '  return cfg.secret + 1;',
      '}',
      '',
      'export const data = { secret: 42 };',
      ''
    ].join('\n');

    await fixture.writeFile('src/other.ts', OTHER_SOURCE);
    ...
    const boxAfter = await fixture.readFile('src/box.ts');
    expect(boxAfter).toContain('#hidden = 1;');
    expect(boxAfter).toContain('return this.#hidden;');
    expect(boxAfter).not.toContain('#secret');

    // Bug：跨檔 rename 誤改了 other.ts 內完全無關的 `.secret` 屬性存取／欄位，
    // 其型別上與 Box 的私有欄位 #secret 毫無關聯（cfg 是結構型別、data 是普通物件字面量）
    const otherAfter = await fixture.readFile('src/other.ts');
