...
export class TypeScriptDependencyAnalyzer {
   ...
   * 從節點提取依賴資訊
   */
  private extractDependencyFromNode(node: ts.Node): Dependency | null {
   ...
   * 從 import 宣告提取依賴
   */
  private extractFromImportDeclaration(node: ts.ImportDeclaration): Dependency | null {
   ...
   */
  private extractFromExportDeclaration(node: ts.ExportDeclaration): Dependency | null {
    const path = getDependencyPath(node);
    if (!path) {
   ...
   * 從 import = 宣告提取依賴
   */
  private extractFromImportEquals(node: ts.ImportEqualsDeclaration): Dependency | null {
   ...
   * 從 require() 呼叫提取依賴
   */
  private extractFromRequireCall(node: ts.CallExpression): Dependency | null {
   ...
   * 從動態 import() 提取依賴
   */
  private extractFromDynamicImport(node: ts.CallExpression): Dependency | null {
    if (node.arguments.length === 0) {
      return null;
    }
   ...
   * 分析循環依賴
   */
  async analyzeCyclicDependencies(dependencies: Dependency[]): Promise<string[][]> {
    const cycles: string[][] = [];
