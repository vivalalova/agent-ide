...
export class CycleDetector {
  ...
  findStronglyConnectedComponents(graph: DependencyGraph): StronglyConnectedComponent[] {
    ...
    const nodeStates = new Map<string, TarjanNode>();
    const stack: string[] = [];
   ...
   * @returns 複雜度分數
   */
  private calculateCycleComplexity(graph: DependencyGraph, cycleNodes: string[]): number {
   ...
   * @returns 合併後的選項
   */
  private getDefaultOptions(options?: CycleDetectionOptions): CycleDetectionOptions {
    return {
      maxCycleLength: 20,
