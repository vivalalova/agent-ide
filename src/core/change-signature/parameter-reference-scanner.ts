...
export class ParameterReferenceScanner {
  ...
  private findReferencedParameterNameInExpression(
    ...
    visitValueIdentifierReferences(initializer, names, (node) => {
  ...
  private rewriteExpressionTextForRename(
    ...
    visitValueIdentifierReferences(expression, names, (node) => {
  ...
  private forEachParameterReference(
    ...
    for (const parameter of targetFunction.parameters) {
      if (parameter.initializer) {
        visitValueIdentifierReferences(parameter.initializer, names, (node) => onReference(node, 'parameter-default'));
      }
    }
  }
  ...
  private forEachBodyIdentifierReference(
    ...
    ts.forEachChild(body, (child) => visitValueIdentifierReferences(child, names, onReference));
  }
