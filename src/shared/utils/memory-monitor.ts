...
export class MemoryMonitor {
  ...
  async cleanup(): Promise<void> {
    ...
    await Promise.allSettled(
      disposableArray.map(async (disposable) => {
        try {
 ...
 * 記憶體使用監控裝飾器
 */
export function withMemoryMonitoring<T extends Disposable>(target: T): T {
 ...
 * 取得格式化的記憶體使用報告
 */
export function getFormattedMemoryReport(): string {
