...
export class RefactorWorkflow extends BaseWorkflow {
    ...
    defineSteps() {
        // 步驟 1: 檔案分析
        this.addStep(this.createStep('analyze-file', '分析檔案結構和複雜度', this.analyzeFile.bind(this), {
            canRetry: true,
            maxRetries: 2
        }));
        // 步驟 2: 重構前驗證
        this.addStep(this.createStep('pre-validation', '驗證重構操作的可行性', this.preValidation.bind(this), {
            canRetry: true,
            maxRetries: 1
        }));
        // 步驟 3: 執行重構
        this.addStep(this.createStep('execute-refactor', '執行重構操作', this.executeRefactor.bind(this), {
            rollback: this.rollbackRefactor.bind(this),
            canRetry: false // 重構操作不重試，避免副作用
        }));
        // 步驟 4: 重構後驗證
        this.addStep(this.createStep('post-validation', '驗證重構結果的正確性', this.postValidation.bind(this), {
            canRetry: true,
            maxRetries: 1
        }));
    }
    ...
    async initializeContext(input) {
        if (!this.isValidInput(input)) {
            throw new WorkflowError('無效的重構工作流程輸入', { input });
        }
        ...
        try {
            ...
            throw new WorkflowError(`無法讀取檔案: ${refactorInput.filePath}`, { filePath: refactorInput.filePath }, error);
        }
        return {
    ...
    async analyzeFile(context) {
        try {
            const workflowContext = context.previousResults.get('context') ||
            ...
            return this.createFailureResult(new WorkflowError(`檔案分析失敗: ${error.message}`, { stepId: context.stepId }, error));
        }
    }
    /**
     * 步驟 2: 重構前驗證
    ...
    async preValidation(context) {
        try {
            ...
            // 驗證重構操作的可行性
            const validationResult = this.validateRefactorOperation(workflowContext.options, analysisResult);
            if (!validationResult.isValid) {
                return this.createFailureResult(new WorkflowError(`重構操作驗證失敗: ${validationResult.reason}`, { validation: validationResult }));
            }
            workflowContext.validationResult = validationResult;
            return this.createSuccessResult(validationResult);
        }
        catch (error) {
            return this.createFailureResult(new WorkflowError(`重構前驗證失敗: ${error.message}`, { stepId: context.stepId }, error));
        }
    }
    /**
     * 步驟 3: 執行重構
     */
    async executeRefactor(context) {
        try {
            const workflowContext = await this.getContextFromMetadata(context);
            // 使用 ModuleCoordinator 執行重構
            const refactorResult = await context.moduleCoordinator.analyzeAndRefactor(workflowContext.filePath, workflowContext.options);
            workflowContext.refactorResult = refactorResult;
            if (!refactorResult.success) {
                return this.createFailureResult(refactorResult.error || new WorkflowError('重構操作失敗'));
            }
            return this.createSuccessResult(refactorResult);
        }
        catch (error) {
            return this.createFailureResult(new WorkflowError(`重構執行失敗: ${error.message}`, { stepId: context.stepId }, error));
        }
    }
    /**
     * 步驟 4: 重構後驗證
     */
    async postValidation(context) {
        try {
            const workflowContext = await this.getContextFromMetadata(context);
            const refactorResult = context.previousResults.get('execute-refactor');
            if (!refactorResult || !refactorResult.success) {
                return this.createFailureResult(new WorkflowError('無法驗證失敗的重構結果'));
            }
            // 驗證重構後的程式碼
            const validationResult = await this.validateRefactoredCode(workflowContext.filePath, refactorResult);
            workflowContext.finalValidationResult = validationResult;
            if (!validationResult.isValid) {
                return this.createFailureResult(new WorkflowError(`重構後驗證失敗: ${validationResult.reason}`, { validation: validationResult }));
            }
            return this.createSuccessResult(validationResult);
        }
        catch (error) {
            return this.createFailureResult(new WorkflowError(`重構後驗證失敗: ${error.message}`, { stepId: context.stepId }, error));
        }
    }
    /**
     * 回滾重構操作
     */
    async rollbackRefactor(context) {
        try {
            const workflowContext = await this.getContextFromMetadata(context);
     ...
     * 驗證重構操作
     */
    validateRefactorOperation(options, analysisResult) {
        // 基本驗證
        if (!options.type) {
     ...
     * 驗證重構後的程式碼
     */
    async validateRefactoredCode(filePath, refactorResult) {
        try {
            // 檢查重構結果
            ...
            return { isValid: false, reason: `驗證失敗: ${error.message}` };
        }
    }
}
//# sourceMappingURL=refactor-workflow.js.map