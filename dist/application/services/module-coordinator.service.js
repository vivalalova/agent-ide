 ...
 */
export class ModuleCoordinatorService {
    eventBus;
    stateManager;
    errorHandler;
    modules;
    ...
    searchService;
    indexEngine;
    constructor(eventBus, stateManager, errorHandler) {
     ...
     * 分析並重構
     */
    async analyzeAndRefactor(filePath, options) {
        const context = {
        ...
        try {
            ...
            switch (options.type) {
                case 'extract-function':
                    if (options.selection && options.newName) {
                        try {
                            ...
                            const extractResult = await this.functionExtractor.extract(code, options.selection, {
                ...
                case 'inline-function':
                    if (options.selection) {
                        try {
                            ...
                            // 這裡簡化處理，需要先找到函式定義和調用
                            const functionDef = { name: 'temp', body: '', parameters: [], range: options.selection };
                            const functionCalls = [];
                ...
                case 'rename':
                    ...
                    break;
                default:
                    throw new ModuleCoordinatorError(`不支援的重構類型: ${options.type}`, { refactorType: options.type });
            }
            // 3. 發送模組協調事件
            ...
            const handledError = await this.errorHandler.handle(error, context);
            return {
                success: false,
                changes: [],
                error: handledError
     ...
     * 發送模組事件
     */
    async emitModuleEvent(eventType, data) {
        try {
    ...
    generatePreview(changes) {
        ...
        const previews = changes.map(change => `檔案: ${change.filePath}\n變更:\n${change.newContent.slice(0, 200)}...`);
        return previews.join('\n\n');
    }
}
//# sourceMappingURL=module-coordinator.service.js.map