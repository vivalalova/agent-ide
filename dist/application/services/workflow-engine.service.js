...
export class WorkflowEngineService {
    ...
    executionPromises;
    workflowRegistry;
    constructor(eventBus, errorHandler, stateManager) {
    ...
    async resume(workflowId) {
        ...
        return await this.continueExecution(workflow, workflowState);
    }
    /**
     * 回滾工作流程
     */
    async rollback(workflowId, stepId) {
        const workflowState = this.activeWorkflows.get(workflowId);
        if (!workflowState) {
            throw new WorkflowError(`Workflow not found: ${workflowId}`);
        }
     ...
     * 執行工作流程的核心邏輯
     */
    async executeWorkflow(workflow, workflowState) {
        const startTime = Date.now();
        let lastResult = workflow.context;
        try {
            workflowState.status = WorkflowStatus.Running;
            // 執行步驟
            ...
            workflowState.status = WorkflowStatus.Failed;
            workflowState.error = error instanceof BaseError ? error : new WorkflowError('Workflow execution failed', { workflowId: workflow.id }, error);
            workflowState.endTime = new Date();
            ...
            const handledError = await this.errorHandler.handle(error, errorContext);
            return {
                workflowId: workflow.id,
                status: WorkflowStatus.Failed,
                error: handledError,
     ...
     * 繼續執行暫停的工作流程
     */
    async continueExecution(workflow, workflowState) {
        // 找到當前應該執行的步驟
     ...
     * 執行單個步驟
     */
    async executeStep(step, context, workflowId) {
        let attemptCount = 0;
        ...
        while (attemptCount <= maxRetries) {
            try {
                ...
                if (!step.canRetry || attemptCount > maxRetries) {
                    const stepError = error instanceof BaseError ? error : new WorkflowError(`Step ${step.id} failed`, { stepId: step.id, workflowId, attempt: attemptCount }, error);
                    return {
                        success: false,
                        error: stepError
                    };
     ...
     * 執行回滾
     */
    async performRollback(workflow, workflowState, targetStepId) {
        ...
        for (const stepId of rollbackSteps) {
            ...
            if (step?.rollback) {
                try {
                    ...
                    throw new WorkflowError(`Rollback failed for step ${stepId}`, { stepId, workflowId: workflow.id }, error);
                }
            }
        }
    }
     ...
     * 發布工作流程事件
     */
    async publishWorkflowEvent(workflowId, eventType, stepId, data) {
        try {
