...
import { createAST, createASTMetadata, ReferenceType, DependencyType, createDependency, createPosition, createLocation, createRange as createCoreRange } from '../../shared/types/index.js';
import { SwiftNodeType, SwiftSymbolType, SwiftParseError, DEFAULT_SWIFT_COMPILER_OPTIONS, createSwiftASTNode, createSwiftSymbol, createSwiftParseError, isValidSwiftIdentifier, isDeclarationNode } from './types.js';
/**
 * Swift Parser 實作
...
export class SwiftParser {
    ...
    initialized = false;
    parser = null; // web-tree-sitter Parser
    language = null; // Swift language
    constructor() {
        // 初始化會在第一次使用時進行
    }
    /**
    ...
    async parse(code, filePath) {
        ...
        try {
            ...
            const metadata = createASTMetadata('swift', this.version, { compilerOptions: DEFAULT_SWIFT_COMPILER_OPTIONS }, Date.now(), 0 // 會在 createAST 中計算
            );
     ...
     * 基本的程式碼解析（暫時實作）
     */
    parseCodeBasic(code, filePath) {
        // 處理空檔案
        if (!code.trim()) {
            const emptyRange = createCoreRange(createPosition(1, 1, 0), createPosition(1, 1, 0));
     ...
     * 從節點提取符號
     */
    extractSymbolsFromNode(node, sourceFile, symbols) {
        ...
        for (const child of node.children) {
            this.extractSymbolsFromNode(child, sourceFile, symbols);
        }
    }
    /**
     * 從節點建立符號
     */
    createSymbolFromNode(node, sourceFile) {
    ...
    async findReferences(ast, symbol) {
        ...
        if (!sourceCode) {
            ...
            for (const child of swiftAST.root.children) {
                if (child.properties.code) {
                    sourceCode = child.properties.code;
                    break;
                }
            }
        }
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                while ((index = line.indexOf(symbolName, index)) !== -1) {
                    ...
                    if (!/\w/.test(beforeChar) && !/\w/.test(afterChar)) {
                        ...
                        const range = createCoreRange(start, end);
                        // 檢查是否與定義位置相同（避免重複）
                        const isDuplicate = range.start.line === symbol.location.range.start.line &&
     ...
     * 從節點提取依賴關係
     */
    extractDependenciesFromNode(node, sourceFile, dependencies) {
        if (node.swiftType === SwiftNodeType.Import) {
            // 基本實作：假設 import 語句
            const importName = this.extractImportName(node);
            if (importName) {
                dependencies.push(createDependency(importName, DependencyType.Import, false, // Swift imports 通常不是相對路徑
                []));
            }
        }
        // 遞歸處理子節點
        for (const child of node.children) {
            this.extractDependenciesFromNode(child, sourceFile, dependencies);
        }
    }
    /**
     * 提取 import 名稱
    ...
    async findSymbolAtPosition(ast, position) {
        ...
        const sameLine = symbols.filter(s => s.location.range.start.line === position.line ||
        ...
        if (sameLine.length > 0) {
            ...
            for (const symbol of sameLine) {
                const distance = Math.min(Math.abs(symbol.location.range.start.column - position.column), Math.abs(symbol.location.range.end.column - position.column));
     ...
     * 從位置提取標識符名稱
     */
    extractIdentifierAtPosition(ast, position) {
        const swiftAST = ast;
        // 從 AST 的源文件中提取標識符
    ...
    swiftSymbolTypeToDefinitionKind(symbolType) {
        switch (symbolType) {
