...
import { SymbolType, createScope } from '../../shared/types/index.js';
import { SYMBOL_TYPE_MAP, getNodeName, getNodeModifiers, tsNodeToRange, isSymbolDeclaration } from './types.js';
/**
 * TypeScript 符號提取器類別
 */
export class TypeScriptSymbolExtractor {
     ...
     * 調整特殊情況的符號
     */
    adjustSymbolForSpecialCases(node, modifiers) {
        // 處理可選屬性
     ...
     * 建構子節點轉換為簽名字串
     */
    constructorNodeToSignature(node) {
        const parameters = node.parameters.map(param => {
            const paramName = param.name.getText(this.sourceFile);
            const paramType = param.type ? `: ${param.type.getText(this.sourceFile)}` : '';
            const optional = param.questionToken ? '?' : '';
