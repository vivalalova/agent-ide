...
export class ImportResolver {
     ...
     */
    analyzeImports(filePath, code) {
        return this.parseImportStatements(code, filePath);
    }
    /**
     * 解析程式碼中的 import 語句
     */
    parseImportStatements(code, filePath) {
        const statements = [];
     ...
     * 更新 import 路徑
     */
    updateImportPath(importStatement, oldFilePath, newFilePath) {
        ...
        try {
            ...
            // 更新 import 語句
            const newStatement = rawStatement.replace(new RegExp(`['"\`]${this.escapeRegExp(importPath)}['"\`]`), `'${newImportPath}'`);
            return {
                filePath: oldFilePath,
                line: position.line,
                oldImport: rawStatement,
     ...
     */
    createImportStatement(type, importPath, lineNumber, columnIndex, rawStatement) {
