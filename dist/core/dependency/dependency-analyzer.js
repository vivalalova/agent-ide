...
export class DependencyAnalyzer {
     ...
     * @returns 傳遞依賴列表
     */
    getTransitiveDependencies(filePath, options) {
        const normalizedPath = path.resolve(filePath);
    ...
    getImpactAnalysis(filePath) {
        ...
        // 計算影響分數
        const impactScore = this.calculateImpactScore(directlyAffected.length, transitivelyAffected.length, affectedTests.length);
        return {
     ...
     * @returns 依賴列表
     */
    async extractDependencies(content, filePath) {
        const dependencies = [];
     ...
     * @returns 解析結果
     */
    async resolvePath(importPath, fromFile) {
     ...
     */
    updateDependencyGraph(fileDependencies) {
     ...
     * @returns 是否包含
     */
    shouldIncludeDependency(resolvedPath) {
     ...
     * @returns 影響分數
     */
    calculateImpactScore(directAffected, transitiveAffected, testAffected) {
