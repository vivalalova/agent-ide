...
export class UnreachableCodeDetector {
     ...
     * 遍歷 AST 檢測不可達代碼
     */
    traverse(node, isUnreachable, unreachable) {
        if (!node) {
            return;
        }
        // 如果已經在不可達狀態，標記所有後續節點
...
export class DeadCodeDetector {
    unusedSymbolDetector = new UnusedSymbolDetector();
    unreachableCodeDetector = new UnreachableCodeDetector();
    /**
     * 檢測檔案中的死代碼
     * @param filePath 檔案路徑
     * @param content 檔案內容
     * @returns 死代碼檢測結果
     */
    async detectInFile(filePath, content) {
        // 輸入驗證
     ...
     * 查找符號引用
     */
    findReferences(content, symbolName) {
        const references = [];
