 ...
 * 負責執行符號重新命名操作
 */
import { ConflictType, createRenameOperation, createConflictInfo } from './types.js';
...
export class RenameEngine {
     ...
     * 查找符號的所有引用
     */
    async findReferences(filePaths, symbol, position) {
        const references = [];
        // 使用簡單的文字匹配來查找引用
        try {
    ...
    async rename(options) {
        ...
        try {
            ...
            const updateResult = await this.referenceUpdater.updateReferences(options.symbol, options.newName, Array.from(options.filePaths));
            ...
            for (const updatedFile of updateResult.updatedFiles) {
                ...
                for (const change of updatedFile.changes) {
                    operations.push(createRenameOperation(updatedFile.filePath, change.oldText, change.newText, change.range));
                }
            }
     ...
     * 驗證重新命名操作的有效性
     */
    async validateRename(options) {
        this.validateOptions(options);
        const conflicts = [];
        // 檢查是否為保留字
        if (this.reservedKeywords.has(options.newName)) {
            conflicts.push(createConflictInfo(ConflictType.ReservedKeyword, `'${options.newName}' 是保留字，不能用作識別符`, options.symbol.location));
        }
        // 檢查是否為有效識別符
        if (!this.isValidIdentifier(options.newName)) {
            conflicts.push(createConflictInfo(ConflictType.InvalidIdentifier, `'${options.newName}' 不是有效的識別符`, options.symbol.location));
        }
        return {
    ...
    async previewRename(options) {
        ...
        const operations = validation.isValid && hasLocation ? [
            createRenameOperation(options.symbol.location.filePath, options.symbol.name, options.newName, options.symbol.location.range)
        ] : [];
        const summary = {
    ...
    async undo(renameId) {
        ...
        try {
            // 建立反向操作來撤銷變更
            const undoOperations = originalResult.operations.map(op => createRenameOperation(op.filePath, op.newText, // 交換新舊文字
            op.oldText, op.range));
            // 執行撤銷操作
     ...
     * 檢測命名衝突
     */
    detectConflicts(newName, scope) {
        const conflicts = [];
        // 檢查保留字
        if (this.reservedKeywords.has(newName)) {
            conflicts.push(createConflictInfo(ConflictType.ReservedKeyword, `'${newName}' 是保留字`, { filePath: '', range: createRange(createPosition(1, 1), createPosition(1, 1)) }));
        }
        // 檢查是否為有效識別符
        if (!this.isValidIdentifier(newName)) {
            conflicts.push(createConflictInfo(ConflictType.InvalidIdentifier, `'${newName}' 不是有效的識別符`, { filePath: '', range: createRange(createPosition(1, 1), createPosition(1, 1)) }));
        }
     ...
     * 執行跨檔案重新命名
     */
    async renameAcrossFiles(symbol, newName, projectFiles) {
        try {
            // 先驗證重新命名的有效性
            ...
            const updateResult = await this.referenceUpdater.updateCrossFileReferences(symbol, newName, projectFiles);
            if (!updateResult.success) {
                return {
                    success: false,
                    operations: [],
