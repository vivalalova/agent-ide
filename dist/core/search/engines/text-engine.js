...
export class TextSearchEngine {
     ...
     * 建立匹配上下文
     */
    buildContext(lines, lineIndex, contextLines) {
        const before = [];
        const after = [];
        // 獲取前面的行
     ...
     * 尋找包圍的類別名稱
     */
    findEnclosingClass(lines, lineIndex) {
        // 簡單的類別檢測正則表達式
     ...
     * 計算字符偏移量
     */
    calculateOffset(lines, lineIndex, columnIndex) {
        let offset = 0;
        // 加上前面所有行的長度（包括換行字符）
     ...
     * 計算匹配分數
     */
    calculateScore(match, query, line) {
        let score = 0;
        // 1. 精確匹配得分更高
        if (match === query) {
            score += 1.0;
     ...
     * 獲取要搜尋的檔案列表
     */
    async getSearchFiles(scope, options) {
        let searchPath;
        let pattern;
        switch (scope.type) {
            case 'file':
        ...
        if (options.includeFiles && options.includeFiles.length > 0) {
            ...
            files = files.filter(file => includePatterns.some(pattern => pattern.test(file)));
        }
        // 應用排除過濾器
