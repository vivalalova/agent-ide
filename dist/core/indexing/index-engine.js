...
import { createFileInfo, shouldIndexFile, calculateProgress } from './types.js';
...
export class IndexEngine {
    ...
    async indexDirectory(dirPath) {
        ...
        // 使用 glob 模式查找檔案
        const includePatterns = this.config.includeExtensions.map(ext => `**/*${ext}`);
        const allFiles = [];
        ...
        const filesToIndex = uniqueFiles.filter(file => shouldIndexFile(file, this.config));
        // 批次索引檔案
     ...
     * 根據類型查找符號
     */
    async findSymbolByType(type, options) {
     ...
     * 批次索引檔案
     */
    async batchIndexFiles(files, options) {
     ...
     * 從檔案統計資訊建立 FileInfo
     */
    async createFileInfoFromStat(filePath, stat) {
        const extension = path.extname(filePath);
        const language = this.getLanguageFromExtension(extension);
        // 計算檔案 checksum
        const content = await fs.readFile(filePath, 'utf-8');
        const checksum = createHash('sha256').update(content).digest('hex');
        return createFileInfo(filePath, stat.mtime, stat.size, extension, language, checksum);
    }
    /**
     * 根據副檔名判斷語言
     */
