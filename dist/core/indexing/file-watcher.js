...
export class FileWatcher extends EventEmitter {
    ...
    debounceTimer = null;
    debounceTime;
    constructor(indexEngine, options) {
        super();
    ...
    async start() {
        ...
        try {
            ...
            this.watcher = watch(config.workspacePath, { recursive: true }, (eventType, filename) => {
     ...
     * 批次處理檔案變更
     */
    async handleBatchChanges(changes, options) {
        const maxConcurrency = options?.maxConcurrency ?? 4;
        const chunks = this.createChunks(changes, maxConcurrency);
        for (const chunk of chunks) {
            const promises = chunk.map(change => this.handleFileChange(change.filePath, change.type));
     ...
     * 將變更加入佇列（帶防抖動）
     */
    queueChange(filePath, changeType) {
        // 檢查檔案是否應該被索引
        const config = this.indexEngine.getConfig();
    ...
    async processFileChange(filePath, changeType) {
        switch (changeType) {
            ...
            case 'unlinkDir':
                ...
                const filesToRemove = allFiles.filter(f => f.filePath.startsWith(filePath + '/'));
