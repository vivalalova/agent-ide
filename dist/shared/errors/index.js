...
export function createError(type, message, options = {}) {
    ...
    switch (type) {
        case 'parser': {
            ...
            return new ParserError(message, options.location, code || 'PARSER_ERROR', options.syntaxElement, cause);
        }
        case 'file': {
            if (!options.filePath) {
                throw new Error('FileError 需要 filePath 參數');
            }
            return new FileError(message, options.filePath, code || 'FILE_ERROR', options.operation, cause);
        }
        case 'validation': {
            if (!options.field) {
                throw new Error('ValidationError 需要 field 參數');
            }
            return new ValidationError(message, options.field, code || 'VALIDATION_ERROR', options.value, cause);
        }
        case 'config': {
            if (!options.configPath) {
                throw new Error('ConfigError 需要 configPath 參數');
            }
            return new ConfigError(message, options.configPath, code || 'CONFIG_ERROR', options.expectedType, cause);
        }
        default:
